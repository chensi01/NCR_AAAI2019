function out = MF_NCR(seed,rawData,file_folder,varargin)
rng(seed);
%% Parse parameters
params = inputParser;
...
for i = 1:cvObj.NumTestSets
    ...
    filename_1 = sprintf('MF_NCR_fold_%i.mat',i);
    filename = fullfile(file_folder,filename_1);
    save (filename,'testIdx','U','V','theta','par','-mat');
    fprintf('MF_NCR %d/%d fold completed\n',i,cvObj.NumTestSets);
end
fprintf('AUC = %f, NDCG = %f, MRR = %f \n',mean(out));
result = sprintf('MF_NCR Final Results: AUC = %f, NDCG = %f, MRR = %f \n',mean(out));
filename = fullfile(file_folder,'MF_NCR_final_result.mat');
