...
params.addParameter('F',5,@(x) isnumeric(x));%10
params.addParameter('lr',5,@(x) isnumeric(x));%50
params.addParameter('regU',0.3,@(x) isnumeric(x));%0.01%0.2
params.addParameter('regV',0.3,@(x) isnumeric(x));%0.01
...
for i = 1:cvObj.NumTestSets
    ...
    toc
    filename = sprintf('BPR_fold_%i_movielens_reg0.3.mat',i);
    save (filename,'testIdx','U','V','par','-mat');
    out(i,:) = BPRRankEval(session,testIdx,U,V,par);
    fprintf('BPR_baseline:fold [%d/%d] completed\n',i,cvObj.NumTestSets);
end
fprintf('Final Results: map = %f, NDCG = %f, oPrec = %f, auc = %f, MRR = %f\n',mean(out));
save 'baseline_result_ml.mat' out
end
